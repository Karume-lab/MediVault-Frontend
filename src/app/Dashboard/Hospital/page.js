...
export default function Hospital() { 
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await fetch('http://localhost:3000/api/hospitalhttp://bkyz2-fmaaa-aaaaa-qaaaq-cai.localhost:4943/patient-profiles')
       ...
       <div className="bg-cover h-screen w-screen bg-no-repeat pt-0 bg-no-scrol" style={{backgroundImage: "url('/background.jpeg')"}}>
            ...
            <div className=' flex flex-col '>
                <div className='flex flex-row justify-start items-center text-black font-bold pt-4 ml-28'>
                    <h3 className='p-1  rounded-b-md border-b-2  border-black shadow-lg text-2xl bg-black bg-opacity-20 text-white '>Registered Patients :</h3>
                </div>
                <div>
                    ...
                    <table className="table-auto border-collapse w-5/6 ml-28 bg-black bg-opacity-25 h-5/6 p-2 m-6 ">
                        <thead>
                            <tr className="rounded-lg text-sm font-medium text-gray-700 text-left" style={{fontSize: '0.9674rem'}}>
                                ...
                                <th className="px-4 py-2 " style={{backgroundColor: '#f8f8f8'}}>Last Appointment</th>
                            </tr>
                        </thead>
                        <tbody className="text-sm font-normal text-gray-700">
                            {patients.map((patient) => (
                                <tr className="hover:bg-blue-400 border-b border-gray-200 py-10 font-bold text-white"key={patient.id}>
                                    ...
                                    <td className="px-4 py-4">{patient.lastAppointment}</td>
                                </tr>
                            ))}
                            <tr className="hover:bg-blue-400 border-b border-gray-200 py-10 font-bold text-white">
                                ...
                                <td className="px-4 py-4">12/12/2021</td>
                            </tr>
                             <tr className="hover:bg-blue-400 border-b border-gray-200 py-10 font-bold text-white">
                                <td className="px-4 py-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
